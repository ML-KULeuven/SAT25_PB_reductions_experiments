...
class MemoryAwareExecutor:
    def __init__(self, timeout):
        self.timeout = timeout
        self.phases = [
            {'limit_gb': 8, 'retries': 2},
            {'limit_gb': 16, 'retries': 1},
    ...
    def run_job(self, job, limit_gb):  # limit_gb comes from current phase
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(delete=False) as temp_file:
                ...
                with bz2.open(instance_path, 'rb') as xz_file:
    ...
    def execute_phase(self, jobs, phase_idx):
        ...
        with ProcessPoolExecutor(max_workers=self.calculate_parallelism(limit_gb)) as executor:
            futures = {
    ...
    def _wrap_run_job(self, job_data, limit_gb):
        ...
        return self.run_job(job_data, limit_gb)
...
def generate_output_file_name(solver_name, config_name, seed, subdir):
    ...
    return f"{folder_path}/seed{seed}.csv"
...
EXPERIMENT_GROUPS = {
    ...
    'WS+AW': {
        'exact': ['base+WS', 'base+AW', 'base+AW+WS']
    },
    'all_options': {
...
EXACT_HEADER = header = ["filename", "elapsed time", "result",
                         "cpu time", "parse time", "solve time", "solve time det", "optimization time", "top-down time",
                         "top-down time det",
                         "bottom-up solve time", "bottom-up solve time det", "conflict analysis time",
                         "learned minimize time", "propagation time",
                         "constraint cleanup time", "inprocessing time", "garbage collection time",
                         "constraint learning time",
                         "time spent in activity heuristic", "at-most-one detection time",
                         "at-most-one detection time det",
                         "time spent in lift degree optimization", "number of lifted degrees", "LP solve time",
                         "LP total time", "LP total time det",
                         "cores", "solutions", "propagations", "decisions", "conflicts", "restarts",
                         "inprocessing phases", "original variables",
                         "auxiliary variables", "input clauses", "input cardinalities", "input general constraints",
                         "input length average",
                         "input degree average", "input strength average", "learned clauses", "learned cardinalities",
                         "learned general constraints",
                         "learned length average", "learned degree average", "learned strength average",
                         "learned LBD average", "unit literals derived",
                         "pure literals", "constraints satisfied at root",
                         "constraints simplified during database reduction", "small coef constraints",
                         "large coef constraints", "arbitrary coef constraints", "probing calls",
                         "probing inprocessing time", "unit lits due to probing",
                         "equalities due to probing", "implications added due to probing",
                         "max implications in memory due to probing",
                         "detected at-most-ones", "units derived during at-most-one detection", "resolve steps",
                         "self-subsumptions",
                         "gcd simplifications", "detected cardinalities", "weakened non-implied",
                         "weakened non-implying",
                         "number of multiply-weakens on reason", "number of multiply-weakens on conflict",
                         "number of direct multiply-weakens",
                         "number of indirect multiply-weakens", "clausal propagations", "cardinality propagations",
                         "watched propagations",
                         "counting propagations", "watch lookups", "watch backjump lookups", "watch checks",
                         "propagation checks",
                         "blocking literal success", "blocking literal fails", "literal additions", "saturation steps",
                         "unknown literals rounded up",
                         "trail pops", "formula constraints", "dominance breaking constraints", "learned constraints",
                         "bound constraints",
                         "core-guided constraints", "reduced constraints", "encountered formula constraints",
                         "encountered dominance breaking constraints",
                         "encountered learned constraints", "encountered bound constraints",
                         "encountered core-guided constraints",
                         "encountered reduced constraints", "encountered detected at-most-ones",
                         "encountered detected equalities",
                         "encountered detected implications", "CG unit cores", "CG non-clausal cores",
                         "best upper bound", "best lower bound",
                         "LP relaxation objective", "LP constraints added", "LP constraints removed", "LP pivots",
                         "LP approximate operations",
                         "LP literal additions", "LP calls", "LP optimalities", "LP no pivot count",
                         "LP infeasibilities", "LP Farkas constraints",
                         "LP dual constraints", "LP basis resets", "LP cycling count", "LP singular count",
                         "LP no primal count", "LP no dual count",
                         "LP no farkas count", "LP other issue count", "LP Gomory cuts", "LP learned cuts",
                         "LP deleted cuts",
                         "LP encountered Gomory constraints", "LP encountered Farkas constraints",
                         "LP encountered dual constraints"
                         ]
...
def main():
    ...
    benchmarks = []
    for subdir in subdirs:
        path = os.path.join(benchmark_base_dir, subdir)
        subsubdirectories = [
            os.path.join(path, d)
        ...
        for subsub in subsubdirectories:
            # print(subsub)
            instances = glob.glob(os.path.join(subsub, "*"))
            # print(instances)
            benchmarks.extend(instances[:10])
        # benchmarks.extend(glob.glob(f"{benchmark_base_dir}{subdir}/*.xz")[:10])
    print(benchmarks)
    if not benchmarks:
        ...
        for config in solver.flag_configs:
            ...
            for seed in SEEDS:
                ...
                for subdir in subdirs:
                    ...
                    benchmarks = glob.glob(f"{benchmark_base_dir}{subdir}/*/*.bz2")
